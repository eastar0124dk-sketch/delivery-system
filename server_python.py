...
if DATABASE_URL:
    ...
    def init_db():
        ...
        c.execute('''CREATE TABLE IF NOT EXISTS canon_billing_meta (
            key TEXT PRIMARY KEY, value TEXT)''')
        c.execute('''CREATE TABLE IF NOT EXISTS canon_staff_users (
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
      try:
        ...
        if path == '/api/canon-users':
            ...
            try:
                rows = db_fetchall('SELECT id, username, name, created_at FROM staff_users WHERE client_code=? ORDER BY id', ('canon',))
            except Exception as e:
                # staff_users 테이블이 아직 없으면 빈 배열
                print(f'[canon-users] error: {e}')
                rows = []
            return self.send_json({'data': rows})
        ...
        if path == '/api/staff-users':
            ...
            try:
                rows = db_fetchall('SELECT id, client_code, username, name, created_at FROM staff_users WHERE client_code=? ORDER BY id', (client,))
                return self.send_json({'data': rows})
            except Exception as e:
                print(f'[staff-users] error: {e}')
                return self.send_json({'data': [], 'error': str(e)}, 200)

        # ── 메틀러 운송청구서 단건 조회 ──
        m_mt = re.match(r'^/api/mettler-transport/(.+)$', path)
        if m_mt:
            if not self.token_ok(): return self.send_json({'error':'Unauthorized'}, 401)
            period_key = unquote(m_mt.group(1))
            row = db_fetch('SELECT * FROM mettler_transport_billing WHERE period_key=?', (period_key,))
            if not row: return self.send_json({'data': [], 'meta': None})
            try:
                return self.send_json({
                ...
                return self.send_json({'data': [], 'meta': None})

        # ── 메틀러 운송청구서 전체 목록 ──
        if path == '/api/mettler-transport':
            if not self.token_ok(): return self.send_json({'error':'Unauthorized'}, 401)
    ...
    def do_POST(self):
      try:
        ...
        if p == '/api/staff-users':
            ...
            try:
                db_exec('INSERT INTO staff_users(client_code,username,password,name) VALUES(?,?,?,?)',
        ...
        # ── 메틀러 운송청구서 저장 (POST를 PUT처럼 처리) ──
        m_mt = re.match(r'^/api/mettler-transport/(.+)$', p)
        if m_mt:
            if not self.token_ok(): return self.send_json({'error':'Unauthorized'}, 401)
            period_key = unquote(m_mt.group(1))
            data = body.get('data')
            ...
            if not isinstance(data, list):
                return self.send_json({'error':'data 배열이 필요합니다.'}, 400)
            data_json = json.dumps(data, ensure_ascii=False)
            meta_json = json.dumps(meta, ensure_ascii=False) if meta else None
            exists = db_fetch('SELECT 1 AS x FROM mettler_transport_billing WHERE period_key=?', (period_key,))
            if exists:
                db_exec('UPDATE mettler_transport_billing SET data=?, meta=?, updated_at=CURRENT_TIMESTAMP WHERE period_key=?',
                        (data_json, meta_json, period_key))
            else:
                db_exec('INSERT INTO mettler_transport_billing(period_key,data,meta) VALUES(?,?,?)',
                        (period_key, data_json, meta_json))
            return self.send_json({'success': True})
        ...
        if p == '/api/auth':
            ...
            # 화주별 직원 (ID + PW) — staff_users 테이블에서 조회
            CLIENT_MODES = { 'canon_staff':'canon', 'chanel_staff':'chanel', 'mettler_staff':'mettler' }
            if mode in CLIENT_MODES:
                client_code = CLIENT_MODES[mode]
                # ID 있으면 staff_users에서 검증
                if username:
                    try:
                        user = db_fetch('SELECT * FROM staff_users WHERE client_code=? AND username=?',
                                        (client_code, username))
                        if user and user.get('password') == pw:
                            t = make_token(pw); valid_staff_tokens.add(t)
                            return self.send_json({'token': t, 'role': mode,
                                                   'username': user.get('username'),
                                                   'name': user.get('name')})
                    except Exception as e:
                        print(f'[auth {mode}] db error: {e}')
                # ID 없거나 매칭 실패 → 메틀러는 공통 PW로 fallback
                if mode == 'mettler_staff' and pw == db_staff:
                    t = make_token(pw); valid_staff_tokens.add(t)
    ...
    # ── PUT (do_POST 라우트 위임) ───────────────────────────────────
    def do_PUT(self):
        # PUT은 POST와 동일하게 처리 (메틀러 운송청구서 저장 등)
        return self.do_POST()
    ...
    def do_PATCH(self):
      try:
        ...
        if m_su:
            ...
            try:
                db_exec(f'UPDATE staff_users SET {sets} WHERE id=?', vals)
                return self.send_json({'success': True})
            except Exception as e:
                msg = str(e).lower()
                if 'unique' in msg or 'duplicate' in msg:
        ...
        if m_cu:
            ...
            if not updates: return self.send_json({'error':'수정할 항목이 없습니다.'}, 400)
            sets = ', '.join(f'{k}=?' for k in updates.keys())
            vals = list(updates.values()) + [m_cu.group(1)]
            try:
                db_exec(f'UPDATE canon_staff_users SET {sets} WHERE id=?', vals)
                return self.send_json({'success': True})
            except Exception as e:
                if 'UNIQUE' in str(e):
                    return self.send_json({'error':'이미 존재하는 아이디입니다.'}, 400)
    ...
    def do_DELETE(self):
      try:
        ...
        m_su = re.match(r'^/api/staff-users/(\d+)$', p)
        if m_su:
            db_exec('DELETE FROM staff_users WHERE id=?', (m_su.group(1),))
            return self.send_json({'success': True})

        # 캐논 직원 삭제 (구버전 호환)
        m_cu = re.match(r'^/api/canon-users/(\d+)$', p)
        if m_cu:
            db_exec('DELETE FROM staff_users WHERE id=? AND client_code=?', (m_cu.group(1), 'canon'))
            return self.send_json({'success': True})
        ...
        if m_mt:
            period_key = unquote(m_mt.group(1))
            db_exec('DELETE FROM mettler_transport_billing WHERE period_key=?', (period_key,))
            return self.send_json({'success': True})
